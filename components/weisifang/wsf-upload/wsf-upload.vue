<template>
    <view class="wsf-upload-list">
        <tui-col :sm="8" :md="6" :lg="4" :span="4" v-for="(item,index) in uploadLists" :key="index"
            class="wsf-upload-col">
            ...
            imgUpload(tempFilePaths, type) { //type 0 为图片 1为视频
                ...
                Promise.all(uploadImgs) //执行所有需请求的接口
                    .then((results) => {
                        ...
                        uni.hideLoading();
                        if (parseInt(_this.upladFinishNum) + parseInt(_this.upladFailNum) >= parseInt(_this
                                .upladAllNum)) {
